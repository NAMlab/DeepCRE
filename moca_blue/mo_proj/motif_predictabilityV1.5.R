...
##############################################################
PROJECT <- "ArthS0-Arth_0e5-cwm-q1q9_W2"
SPEC <- "Arth"
MODEL <- "S0"
DATE <- "20230809"
...
dirpath_2 <- "./out"
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
file1 <- "ArthS0Arth_0e5-cwm_W2_gene_none20230809-q1q9.csv"
...
file_path_out <- file.path(dirpath_2, paste0(DATE,"_",PROJECT,"_performance"))
 # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
library(stats)
library(dplyr)
library(stringr)
...
merg_df00 <- merged_df[, c("loc_ID", "dist_transc_border", "region", "motif", "gen_mstart", "prob")] # may include "loc"
...
contingency_table_prob_class <- table(a0_fd$motif, a0_fd$prob_class)  # no value if file2 is missing
# Contingency table of "motif" and "performance"
contingency_table_pred_perf <- table(a0_fd$motif, a0_fd$pred_perf)    # no value if file2 is missing
################################################################################
################################################################################
...
#############################
#### if motif is p1 or p0 and represented in a certain expression class should be calculated here
#head(tfestCDE)
tfestCDE$TPR_p0_expr <- tfestCDE$expr_class0 / (tfestCDE$expr_class0 + tfestCDE$expr_class1)
tfestCDE$TPR_p1_expr <-  tfestCDE$expr_class1 / (tfestCDE$expr_class0 + tfestCDE$expr_class1)
tfestCDE$TPR_p0_prob <- tfestCDE$prob_class0 / (tfestCDE$prob_class0 + tfestCDE$prob_class1)
tfestCDE$TPR_p1_prob <-  tfestCDE$prob_class1 / (tfestCDE$prob_class0 + tfestCDE$prob_class1)
tfestCDE$TPR_TF <- tfestCDE$`TRUE` / (tfestCDE$`FALSE` + tfestCDE$`TRUE`)
...
tfestCDE$chi_TF <- apply(tfestCDE[, c("FALSE", "TRUE")], 1, function(row) {
  result <- chisq.test(row, p = c(0.25, 0.75))
  result$p.value
})
###########################################################################
