...
class ConvNetwork:
    def __init__(self, encoded_train_seqs, encoded_val_seqs, train_ids, val_ids, val_chromosome,
                 tpm_counts, organism, case, sequence_size=1000, size_effect=False):
        """
        ...
        :param sequence_size: length of sequence upstream and downstream TSS and TTS respectively
    ...
    def build_network(self, x_train, x_val, y_train, y_val):
        ...
        print(model.summary())
        if self.effect_size:
            model_save_name = f'saved_models/seq_effect_{self.organism}_model_{self.val_chrom}_{self.sequence_size}.h5'
        else:
            model_save_name = f'saved_models/{self.organism}_model_{self.val_chrom}_{self.case}.h5'
        ...
        if self.effect_size:
            performace = [val_acc, val_auroc, self.organism, self.sequence_size, x_train.shape[0]]
        else:
            performace = [val_acc, val_auroc, self.organism, self.case, x_train.shape[0]]

        if model_save_name.endswith('si-nucleotide_shuffle.h5'):
    ...
    def train_network(self):
        ...
        # Masking first three nucleotides after TSS and before TTS
        x_train[:, self.sequence_size:self.sequence_size+3, :] = 0
        x_train[:, self.sequence_size+1017:self.sequence_size+1020, :] = 0
        x_val[:, self.sequence_size:self.sequence_size+3, :] = 0
        x_val[:, self.sequence_size+1017:self.sequence_size+1020, :] = 0
...
def prepare_valid_seqs(fasta, gtf, tpms, val_chrom, pkey=False, upstream=1000, downstream=500):
    ...
    gene_models = gene_models[['Chromosome', 'Start', 'End', 'Strand', 'gene_id']]
    if pkey:
        gene_models = gene_models[gene_models['Chromosome'] == val_chrom]
    ...
    x_val[:, upstream+1017:upstream+1020, :] = 0
