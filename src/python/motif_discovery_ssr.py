...
def compute_actual_and_hypothetical_scores(fasta, gtf, tpms, pkey, specie):
    actual_scores_all, hypothetical_scores_all, onehot_all = [], [], []
    for saved_model in os.listdir('saved_models'):
        if saved_model.startswith(specie):
            print(saved_model)
            val_chrom = saved_model.split('_')[2]
            x_val, y_val, _ = prepare_valid_seqs(fasta, gtf, tpms, val_chrom, pkey)
...
for plant, fasta_file, gtf_file, pickled_key, counts in zip(species, genomes, gene_models, pickle_keys,
                                                            mapped_read_counts):
    if not os.path.exists(f'modisco/tn_{plant}_modisco.hdf5'):
        print(f'Computing contribution and hypothetical contribution scores for {plant}-----------------------------\n')
        compute_actual_and_hypothetical_scores(fasta_file, gtf_file, counts, pickled_key, plant)
