...
for m_reads, gene_model, genome, num_chr, p_key in zip(mapped_read_counts, gene_models, genomes, num_chromosomes,
                                                       pickle_keys):
    for outer_flank in [500, 1500, 2000, 2500, 3000]:
        print('-----------------------------------------------------------------------------\n')
        print(f"Plant: {m_reads.split('_')[0]} Outer flank: {outer_flank}nt inner flank: 500nt")
        print('-----------------------------------------------------------------------------\n')

        if not os.path.exists(f"../results/{m_reads.split('_')[0]}_{outer_flank}_outer_result.csv"):
            ...
            for val_chromosome in np.arange(1, num_chr+1):
                fastaloader = FastaSequenceLoader(f'genomes/{genome}', f'gene_models/{gene_model}',
                                                  ...
                                                  pickled_key=p_key, upstream=outer_flank, downstream=500)
                ...
                convnet = ConvNetwork(enc_train, enc_val, train_ids, val_ids, val_chromosome, tpm_counts,
                                      m_reads.split('_')[0], 'promoter_terminator', outer_flank=outer_flank,
                                      size_effect=True)
                val_acc, val_auroc, specie_name, _, _ = convnet.train_network()
                final_training_output.append([val_acc, val_auroc, specie_name, outer_flank])

            final_training_output = pd.DataFrame(final_training_output, columns=['val_acc', 'val_auROC', 'plant',
                                                                                 'size'])
            final_training_output.to_csv(f"../results/{m_reads.split('_')[0]}_{outer_flank}_outer_result.csv",
                                         index=False)
